...
## v19 fix — Blue hero frame with bottom gap

Η v19 διορθώνει την αρχική μπλε ενότητα ώστε:
- όλο το κείμενο να μένει καθαρά μέσα στο μπλε,
- να υπάρχει ορατή απόσταση περίπου 1 εκατοστό κάτω από το κείμενο,
- να μην πέφτει η περιγραφή πάνω στη λευκή περιοχή.

Για ανέβασμα:
```text
13_PUSH_HERO_BLUE_FRAME_GAP.cmd
```

Generated v19: 2026-05-20T02:15:02


## v20 — Cloudflare Worker activation

Η v20 προσθέτει αυτοματοποιημένη ενεργοποίηση Cloudflare Worker για μετρητές.
...
Το script δημιουργεί/ρυθμίζει:
- Worker `fic-windows-utilities-counters`
- KV namespace `DOWNLOAD_STATS`
- endpoints `/api/visit`, `/api/stats`, `/download/latencycheck`, `/download/harddisktemp`
- ενημέρωση `assets/js/worker-config.js`
- push στο GitHub Pages

Generated v20: 2026-05-20T02:17:26


## v21 fix — Cloudflare Worker npm notice

Η v21 διορθώνει το σταμάτημα στο σημείο:
...
Το `npm notice` δεν είναι πραγματικό σφάλμα deploy. Η v21 κάνει το PowerShell να μη σταματά σε harmless stderr output από `npm/npx/wrangler`.
